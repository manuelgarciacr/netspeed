...
{
	if [[ "$1" != "stopped" ]]; then
		tmux set -g @netspeed-stop "stopped"
	    tmux display-message "Stopping netspeed..."
	    options=$(tmux show -g | grep -E "@netspeed-down|@netspeed-up" | awk '{print $1}')
